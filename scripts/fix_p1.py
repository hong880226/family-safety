"""Fix P1: remove duplicated tasks and reorder properly."""
from pathlib import Path

DOCS = Path("E:/codeRepo/familysafety/docs")
p1 = DOCS / "tasks-p1.md"

CORRECT = """# FamilySafety 任务计划 - P1 后端 MVP
...
p1.write_text(CORRECT, encoding="utf-8")
print(f"  fixed tasks-p1.md ({len(CORRECT)} bytes)")
...
# Count tasks
import re
matches = re.findall(r"^## T\d+", p2_text, re.MULTILINE)
print(f"  P2 has {len(matches)} tasks: {matches}")
print(f"  P2 file: {len(p2_text)} bytes")